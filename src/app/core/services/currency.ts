...
export const getCurrencyRate = async (fromCurrency: string, toCurrency: string): Promise<number> => {
  const result: AxiosResponse = await axios(`${ environment.apiUrl }symbols=${ toCurrency }&base=${ fromCurrency }`);
  if (result.data) {
...
export const getAllCurrencyRates = async (): Promise<CurrencyRate[]> => {
  const result: AxiosResponse = await axios(`${ environment.apiUrl }base=USD`);
